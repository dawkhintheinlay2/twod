...
// FIX: အငြိမ်ဆုံး Model ကိုပဲ သုံးပါမယ် (Error ကင်းအောင်)
const AI_MODELS = ["gemini-1.5-flash", "gemini-1.5-pro"];
  ...
  if (req.method === "POST") {
    if (url.pathname === "/chat") {
        try {
            ...
            let reply = "မိတ်ဆွေရေ၊ အင်တာနက်လိုင်းလေး စစ်ပေးပါဦးခင်ဗျ။";
            let success = false;
            for (const model of AI_MODELS) {
                ...
                try {
                    ...
                    else if (data.error) { reply = "Error: " + data.error.message; } // Show specific error
                } catch (e) {}
            }
