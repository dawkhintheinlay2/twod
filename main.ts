...
serve(async (req) => {
  ...
  if (req.method === "POST") {
    ...
    if (url.pathname === "/bet") {
        ...
        let atomic = kv.atomic().check({ key: ["users", currentUser], versionstamp: (await kv.get(["users", currentUser])).versionstamp }).set(["users", currentUser], { ...uData, balance: balance - (nums.length * amt) });
