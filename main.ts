...
serve(async (req) => {
  ...
  if (!uData) return Response.redirect(url.origin + "/logout"); // Security Check
  ...
  if (req.method === "POST") {
    ...
    if (url.pathname === "/bet") {
        ...
        let atomic = kv.atomic().check({ key: uKey, versionstamp: (await kv.get(uKey)).versionstamp }).set(uKey, { ...uData, balance: balance - (nums.length * amt) });
