...
// --- FORMULA 1 (5/10 Difference + 1) ---
function calculateFormula5_10(twod: string) {
    try {
        if (!twod || twod.includes("-")) return null;
...
async function getBotPredictions(type: string) {
    let baseNum = null;
    let setValData = null;
    ...
    try {
        ...
        if (type === 'morning') {
            // မနက်အတွက်ဆိုရင် မနေ့ညနေက ဂဏန်းလိုတယ်
            // Live မှာ ညနေဂဏန်းရှိမရှိ စစ် (တနင်္လာမနက်ဆို Live မှာ သောကြာညနေဂဏန်း မရှိတတ်ဘူး)
            if (data.result && (data.result[3] || data.result[2])) {
                baseNum = (data.result[3] || data.result[2]).twod;
            }

            // Live မှာမရှိရင် Database History ထဲက နောက်ဆုံးထွက်ခဲ့တဲ့ ညနေဂဏန်းကို ရှာမယ်
            if (!baseNum || baseNum === "--") {
                ...
                for await (const entry of iter) {
                    ...
                    if (val.evening && val.evening !== "--") {
                        baseNum = val.evening;
                        source = `Last Close (${val.date})`;
                    }
                }
            }

            if (baseNum) formula1 = calculateFormula5_10(baseNum);

        } else if (type === 'evening') {
            // ညနေအတွက်ဆိုရင် ဒီနေ့မနက်ဂဏန်း လိုတယ်
            if (data.result && data.result[1]) {
                baseNum = data.result[1].twod;
                source = "Today Morning";
            ...
            if (baseNum && baseNum !== "--") {
                formula1 = calculateFormula5_10(baseNum);
                if (setValData) formula2 = calculateFormulaSetVal(setValData.set, setValData.val);
            }
        }

        return { baseNum, source, formula1, formula2 };
...
Deno.cron("Save History", "*/5 * * * *", async () => {
  try {
    ...
    let m = "--", e = "--";
    if (data.result) { if (data.result[1]?.twod) m = data.result[1].twod; const ev = data.result[3] || data.result[2]; if (ev?.twod) e = ev.twod; }
    if (m !== "--" || e !== "--") {
        const ex = await kv.get(["history", dateKey]); const old = ex.value as any || { morning: "--", evening: "--" };
...
serve(async (req) => {
  ...
  // --- BOT PREDICTION API (FIXED FALLBACK) ---
  if (req.method === "POST" && url.pathname === "/bot_predict") {
      try {
          const { type } = await req.json();
          const prediction = await getBotPredictions(type);
          
          let resultHtml = "";
          const title = type === 'morning' ? 'Morning Prediction (Formula 1)' : 'Evening Prediction';

          if (type === 'morning') {
             if (prediction.baseNum) {
                 const res = prediction.formula1 || "Calculation Error";
                 resultHtml = `
                 <div class="text-center space-y-4">
                    <div class="text-xs text-gray-400">Using: ${prediction.baseNum} (${prediction.source})</div>
                    <div class="bg-yellow-500/10 p-4 rounded-xl border border-yellow-500/50">
                        <div class="text-yellow-500 font-bold text-sm mb-1">FORMULA 1 (5/10)</div>
                        <div class="text-3xl font-bold text-white tracking-[4px]">${res}</div>
                    </div>
                    <div class="text-[10px] text-gray-500">For Tomorrow Morning</div>
                 </div>`;
             } else {
                 resultHtml = `<div class="text-red-400 text-center">No data available yet.</div>`;
             }
          } else {
              if (prediction.baseNum) {
                  const f1 = prediction.formula1 || "Error";
                  const f2 = prediction.formula2 || "Wait for Set/Val";
                  resultHtml = `
                  <div class="text-center space-y-4">
                    <div class="text-xs text-gray-400">Using Morning: ${prediction.baseNum}</div>
                    <div class="bg-yellow-500/10 p-3 rounded-xl border border-yellow-500/50">
                        <div class="text-yellow-500 font-bold text-xs mb-1">FORMULA 1</div>
                        <div class="text-2xl font-bold text-white tracking-[3px]">${f1}</div>
                    </div>
                    <div class="bg-blue-500/10 p-3 rounded-xl border border-blue-500/50">
                        <div class="text-blue-400 font-bold text-xs mb-1">FORMULA 2 (Set/Val)</div>
                        <div class="text-2xl font-bold text-white tracking-[3px]">${f2}</div>
                    </div>
                    <div class="text-[10px] text-gray-500">For This Evening</div>
                  </div>`;
              } else {
                  resultHtml = `<div class="text-red-400 text-center">Morning result not released yet.</div>`;
              }
          }
  ...
  if (req.method === "POST") {
    ...
    if (url.pathname === "/change_password") {
        const form = await req.formData(); const p = form.get("new_password")?.toString();
        if(p) { const s = generateId(); const h = await hashPassword(p, s); await kv.set(["users", currentUser], { ...uData, passwordHash: h, salt: s }); return Response.redirect(url.origin + "/profile?msg=pass_ok"); }
    }
    if (url.pathname === "/clear_history") {
        const iter = kv.list({ prefix: ["bets"] });
        for await (const e of iter) { const b = e.value as any; if(b.user === currentUser && b.status !== "PENDING") await kv.delete(e.key); }
        return new Response(JSON.stringify({status:"ok"}));
    }
    if (url.pathname === "/delete_transaction") {
        const form = await req.formData(); const id = form.get("id")?.toString();
    ...
    if (url.pathname === "/bet") {
        ...
        if (balance < nums.length * amt) return new Response(JSON.stringify({ status: "no_balance" }));
        let atomic = kv.atomic().check({ key: ["users", currentUser], versionstamp: uEntry.versionstamp }).set(["users", currentUser], { ...uData, balance: balance - (nums.length * amt) });
        ...
        for (const n of nums) {
            ...
            atomic = atomic.set(["bets", betId], { user: currentUser, number: n, amount: amt, status: "PENDING", time: txTime, rawMins: mins, batchId, date: dateStr });
        }
  ...
  if (url.pathname === "/bot") {
      return new Response(`
        <!DOCTYPE html><html><head><title>Auto Bot</title>${commonHead.replace(/<style>[\s\S]*?<\/style>/, `<style>
        ...
        <body class="flex flex-col h-screen">
          <div class="bg-slate-900 p-4 shadow-xl border-b border-slate-800 z-10 flex items-center gap-3">
             ...
             <h1 class="font-bold text-lg text-white">Auto Prediction Bot</h1>
          </div>
          <div class="p-6 space-y-4 overflow-y-auto flex-1">
             <div class="text-center mb-6"><i class="fas fa-microchip text-5xl text-blue-500 mb-2"></i><p class="text-gray-400 text-sm">Auto Calculation (No AI)</p></div>
             <div onclick="getPredict('morning')" class="btn-box group">
                 ...
                 <p class="text-xs text-gray-500">Uses Yesterday Evening</p>
             </div>
             <div onclick="getPredict('evening')" class="btn-box group">
                 ...
                 <p class="text-xs text-gray-500">Uses Today Morning</p>
             </div>
          </div>
          <script>
  ...
  // (Include Profile, History, Home Page blocks here - same as previous)
  if (url.pathname === "/profile") {
      ...
      for await (const e of bIter) { const b = e.value as any; if(b.user === currentUser && b.date === dateStr) { if(b.status === 'WIN') todayWin += (b.winAmount || 0); if(b.status === 'LOSE') todayLose += b.amount; } }
      ...
      <div class="p-6 max-w-md mx-auto space-y-4 pb-24">
         <div class="glass p-6 rounded-3xl text-center relative mt-4">
            <div class="relative w-24 h-24 mx-auto mb-3"><div class="w-24 h-24 rounded-full border-4 border-yellow-500 overflow-hidden relative bg-slate-800 flex items-center justify-center">${avatar ? `<img src="${avatar}" class="w-full h-full object-cover">` : `<i class="fas fa-user text-4xl text-gray-500"></i>`}</div><button onclick="document.getElementById('fIn').click()" class="absolute bottom-0 right-0 bg-white text-black rounded-full p-2 border-2 border-slate-900"><i class="fas fa-camera text-xs"></i></button><input type="file" id="fIn" hidden accept="image/*" onchange="upAv(this)"></div>
            <h1 class="text-xl font-bold text-white uppercase">${currentUser}</h1><div class="text-yellow-500 font-mono font-bold text-lg">${balance.toLocaleString()} Ks</div>
         </div>
         <div class="grid grid-cols-2 gap-2 text-center">
            <div class="glass p-3 rounded-xl border-l-2 border-green-500"><div class="text-xs text-gray-400">Today Win</div><div class="font-bold text-green-400 text-sm">+${todayWin.toLocaleString()}</div></div>
            <div class="glass p-3 rounded-xl border-l-2 border-red-500"><div class="text-xs text-gray-400">Today Loss</div><div class="font-bold text-red-400 text-sm">-${todayLose.toLocaleString()}</div></div>
         </div>
      ...
      <script>
        ...
        function showTx(t, a, type) { Swal.fire({ title: 'Transaction', html: \`<div class="text-left">Type: <b>\${type}</b><br>Amount: <b class="text-green-400">\${a} Ks</b><br>Time: \${t}</div>\`, background:'#1e293b', color:'#fff' }); }
        function delTx(e, id) { e.stopPropagation(); Swal.fire({title:'Delete Record?', icon:'warning', showCancelButton:true, confirmButtonColor:'#d33', background:'#1e293b', color:'#fff'}).then(r => { if(r.isConfirmed) { const fd = new FormData(); fd.append('id', id); fetch('/delete_transaction', {method:'POST', body:fd}).then(res=>res.json()).then(d=>{ if(d.status==='ok') location.reload(); }); } }); }
  ...
  return new Response(`
    ...
    <div class="pt-20 px-4 pb-24 max-w-md mx-auto space-y-6">
        ...
        <a href="/bot" onclick="showLoad()" class="block w-full bg-gradient-to-r from-indigo-500 to-purple-600 p-4 rounded-2xl shadow-lg shadow-blue-600/20 flex items-center justify-center gap-3 active:scale-95 transition-transform">
            <div class="bg-white/20 p-2 rounded-full"><i class="fas fa-robot text-xl text-white"></i></div>
            <span class="font-bold text-white">Auto Bot (Prediction)</span>
        </a>
