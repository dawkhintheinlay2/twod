...
const LOADER_HTML = `<div id="loader" class="fixed inset-0 bg-black/90 z-[9999] hidden flex items-center justify-center"><div class="loader w-10 h-10"></div></div>`;
const NAV_HTML = `
<div class="fixed bottom-0 w-full glass border-t border-white/10 pb-safe flex justify-around items-center h-16 z-40">
    <a href="/" onclick="showLoad()" class="nav-item flex flex-col items-center text-gray-400 hover:text-yellow-500"><i class="fas fa-home text-lg"></i><span class="text-[10px] mt-1">Home</span></a>
    <a href="/history" onclick="showLoad()" class="nav-item flex flex-col items-center text-gray-400 hover:text-yellow-500"><i class="fas fa-calendar-alt text-lg"></i><span class="text-[10px] mt-1">History</span></a>
    <a href="/profile" onclick="showLoad()" class="nav-item flex flex-col items-center text-gray-400 hover:text-yellow-500"><i class="fas fa-user-circle text-lg"></i><span class="text-[10px] mt-1">Profile</span></a>
</div>`;
...
serve(async (req) => {
  ...
  if (currentUser && req.method === "POST") {
    ...
    if (url.pathname === "/bet") {
        ...
        const dateStr = now.toLocaleString("en-US", { timeZone: "Asia/Yangon", day: 'numeric', month: 'short', year: 'numeric' });
  ...
  // --- UI RENDERING (LOGIN PAGE) ---
  if (!currentUser) {
    return new Response(`<!DOCTYPE html><html><head><title>Login</title>${COMMON_HEAD}</head><body class="flex items-center justify-center min-h-screen bg-[url('https://images.unsplash.com/photo-1605218427360-36390f8584b0')] bg-cover bg-center">
    <div class="absolute inset-0 bg-black/80"></div>${LOADER_HTML}
  ...
  if (url.pathname === "/profile") {
      ...
      return new Response(`<!DOCTYPE html><html><head><title>Profile</title>${COMMON_HEAD}</head><body>${LOADER_HTML}${NAV_HTML}
  ...
  if (url.pathname === "/history") {
      ...
      return new Response(`<!DOCTYPE html><html><head><title>2D History</title>${COMMON_HEAD}</head><body>${LOADER_HTML}${NAV_HTML}
  ...
  if (url.pathname === "/ai") {
      return new Response(`
        <!DOCTYPE html><html><head><title>Soe Kyaw Win AI</title>${COMMON_HEAD.replace(/<style>[\s\S]*?<\/style>/, `<style>
  ...
  return new Response(`
    <!DOCTYPE html><html><head><title>Home</title>${COMMON_HEAD}</head><body>${LOADER_HTML}
