...
async function getBotPredictions() {
    try {
        ...
        let morningNum = null, eveningNum = null, setValData = null;
        if (data.result && data.result[1]) {
            morningNum = data.result[1].twod;
            if (data.result[1].set && data.result[1].value) setValData = { set: data.result[1].set, val: data.result[1].value };
        }
        if (data.result && (data.result[3] || data.result[2])) eveningNum = (data.result[3] || data.result[2]).twod;
        
        // Fallback for weekends/closed market (Get last history)
        let source = "Live";
        if (!morningNum && !eveningNum) {
             const iter = kv.list({ prefix: ["history"] }, { limit: 1, reverse: true });
             for await (const entry of iter) {
                 const val = entry.value as any;
                 if (val.evening && val.evening !== "--") { eveningNum = val.evening; source = "Last Market Close"; }
             }
        }
        return { morningNum, eveningNum, setValData, source };
    } catch(e) { return {}; }
}
...
async function getAIContext() {
    ...
    try {
        ...
        if(dayName === 'Monday' || dayName === 'Friday') context += `[DAY_INFO]: Today is ${dayName}. Warn about Doubles.\n`;

        if (botData.eveningNum) {
            context += `STATUS: Evening Result (${botData.eveningNum}) is OUT.\n`;
            context += `FORMULA_1 (FOR TOMORROW): [${calculateFormula5_10(botData.eveningNum)}]\n`;
        } else if (botData.morningNum) {
            context += `STATUS: Morning Result (${botData.morningNum}) is OUT.\n`;
            context += `FORMULA_1 (FOR EVENING): [${calculateFormula5_10(botData.morningNum)}]\n`;
            if (botData.setValData) {
                context += `FORMULA_2 (Set/Val - FOR EVENING): [${calculateFormulaSetVal(botData.setValData.set, botData.setValData.val)}]\n`;
            }
        } else { context += `STATUS: Market Not Open Yet.\n`; }

        context += `\n[PAST HISTORY]:\n`;
        const iter = kv.list({ prefix: ["history"] }, { limit: 10, reverse: true });
        for await (const entry of iter) { const val = entry.value as any; context += `${val.date}: ${val.morning}, ${val.evening}\n`; }
...
serve(async (req) => {
        ...
        try {
            ...
            for (const model of AI_MODELS) {
                ...
                try {
                    ...
                    else if (data.error) { reply = "Error: " + data.error.message; }
                } catch (e) {}
            }
    ...
    if (url.pathname === "/bot_predict") {
        try {
            ...
            if (type === 'morning') {
                ...
                let base = botData.eveningNum || botData.morningNum;
                // Fallback if no live data
                if(!base) base = botData.source === "Last Market Close" ? botData.eveningNum : "--";

                if(base && base !== "--") {
                   resultHtml = `<div class="text-center space-y-3"><div class="text-xs text-gray-400">Based on: ${base} (${botData.source})</div><div class="bg-slate-800 p-3 rounded-xl border border-yellow-500/30"><div class="text-yellow-500 font-bold text-xs mb-1">FORMULA 1</div><div class="text-2xl font-bold text-white tracking-widest">${calculateFormula5_10(base)}</div></div><div class="text-[10px] text-gray-500">For Next Morning</div></div>`;
                } else {
                   resultHtml = `<div class="text-center text-red-400">No Data Available</div>`;
                }
            } else {
                title = "Evening Prediction";
                let base = botData.morningNum;
                if(base) {
                    const p1 = calculateFormula5_10(base);
                    const p2 = botData.setValData ? calculateFormulaSetVal(botData.setValData.set, botData.setValData.val) : "Waiting for Set/Val";
                    resultHtml = `<div class="text-center space-y-3"><div class="text-xs text-gray-400">Based on Morning: ${base}</div><div class="bg-slate-800 p-3 rounded-xl border border-yellow-500/30"><div class="text-yellow-500 font-bold text-xs mb-1">FORMULA 1</div><div class="text-2xl font-bold text-white tracking-widest">${p1}</div></div><div class="bg-slate-800 p-3 rounded-xl border border-blue-500/30"><div class="text-blue-400 font-bold text-xs mb-1">FORMULA 2</div><div class="text-2xl font-bold text-white tracking-widest">${p2}</div></div><div class="text-[10px] text-gray-500">For This Evening</div></div>`;
                } else {
                    resultHtml = `<div class="text-center text-red-400">Morning result not out yet.</div>`;
                }
            }
  ...
  return new Response(`
    ...
    <script>
      ...
      return new Response(`<html><body><h1>Internal Server Error</h1><pre>${e.message}</pre></body></html>`, { status: 500, headers: {"content-type": "text/html"} });
  }
});
